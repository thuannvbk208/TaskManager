...
@objc public class DateTimePicker: UIView {
    ...
    internal var years: [String] = [String]()
    ...
    func getYears(_ date: Date) -> [String] {
        var years = [String]()
        let formatter = DateFormatter()
    ...
    @objc open class func show(selected: Date? = nil, minimumDate: Date? = nil, maximumDate: Date? = nil, timeInterval: MinuteInterval = .default) -> DateTimePicker {
        ...
        dateTimePicker.years = dateTimePicker.getYears(currentDate)
    ...
    private func configureView() {
        ...
        // content view
        contentHeight = isDatePickerOnly ? 228 : isTimePickerOnly ? 230 : 470
        ...
        dateTitleLabel.font = UIFont(name: "Avenir", size: 15.0)
        dateTitleLabel.textColor = darkColor
        ...
        cancelButton.titleLabel?.font = UIFont(name: "Avenir-black", size: 15.0)
        ...
        todayButton.titleLabel?.font = UIFont(name: "Avenir-black", size: 15.0)
        ...
        //
        let yearLayout = StepCollectionViewFlowLayout()
        yearLayout.scrollDirection = .horizontal
        yearLayout.minimumInteritemSpacing = 10
        yearLayout.sectionInset = UIEdgeInsets(top: 10, left: 0, bottom: 10, right: 0)
        yearLayout.itemSize = CGSize(width: 75, height: 40)
        
        yearCollectionView = UICollectionView(frame: CGRect(x: 0, y: 44, width: contentView.frame.width, height: 70), collectionViewLayout: yearLayout)
        
        yearCollectionView.backgroundColor = daysBackgroundColor
        yearCollectionView.showsHorizontalScrollIndicator = false
        yearCollectionView.register(YearCollectionViewCell.self, forCellWithReuseIdentifier: "yearCell")
        
        yearCollectionView.dataSource = self
        yearCollectionView.delegate = self
        
        let yearInset = (yearCollectionView.frame.width - 75) / 2
        yearCollectionView.contentInset = UIEdgeInsets(top: 0, left: yearInset, bottom: 0, right: yearInset)
        ...
        monthCollectionView = UICollectionView(frame: CGRect(x: 0, y: 104, width: contentView.frame.width, height: 70), collectionViewLayout: monthLayout)
        ...
        dayCollectionView = UICollectionView(frame: CGRect(x: 0, y: 174, width: contentView.frame.width, height: 100), collectionViewLayout: layout)
        ...
        doneButton.setTitleColor(UIColor(rgb: AppContants.primaryColor), for: .normal)
        doneButton.backgroundColor = doneBackgroundColor ?? darkColor.withAlphaComponent(0.5)
        doneButton.titleLabel?.font = UIFont(name: "Avenir-black", size: 13.0)
        ...
        colonLabel1.text = ":"
        colonLabel1.font = UIFont(name: "Avenir", size: 18.0)
        colonLabel1.textColor = highlightColor
        ...
        colonLabel2.text = ":"
        colonLabel2.font = UIFont(name: "Avenir", size: 18.0)
        colonLabel2.textColor = highlightColor
...
//        contentView.addSubview(separatorBottomView)
    ...
    func updateCollectionView(to currentDate: Date) {
        ...
        //
        formatter.dateFormat = "yyyy"
        let selectedYear = formatter.string(from: currentDate)
        let index = years.firstIndex(of: selectedYear)!
        let idxPath = IndexPath(row: index, section: 0)
        yearCollectionView.scrollToItem(at: idxPath, at: .centeredHorizontally, animated: true)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3, execute: {
            self.yearCollectionView.selectItem(at: idxPath, animated: true, scrollPosition: .centeredHorizontally)
        })
    }
...
//            components.month = dayComponent.month
            ...
            var yearComponent = calendar.dateComponents([.day,.month,.year], from: selectedDate)
            let year = years[indexPath.item]
            yearComponent.year = Int(year)
            if let selected = calendar.date(from: yearComponent) {
                selectedDate = selected
                fillDates(fromDate: selected.startOfMonth(), toDate: selected.endOfMonth())
                minimumDate = selected.startOfMonth()
                maximumDate  = selected.endOfMonth()
